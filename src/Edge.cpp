...
std::pair<int, int> Edge::get_arrow_dim() const  { return m_arrow_dim; }
int Edge::get_arrow_offset() const { return m_arrow_offset; }
bool Edge::get_debug() const { return m_debug; }
...
void Edge::set_arrow_dim(std::pair<int, int> arrow_dim) { m_arrow_dim = arrow_dim; }
void Edge::set_arrow_offset(int arrow_offset) { m_arrow_offset = arrow_offset; }
void Edge::set_debug(bool debug) { m_debug = debug; }
...
void Edge::draw_arrow_circle(ModifiedPGE& engine) const {
    ...
    engine.FillCircle(intersection_coord.first, intersection_coord.second, get_arrow_dim().first, get_color("arrow_bg"));
    engine.DrawCircle(intersection_coord.first, intersection_coord.second, get_arrow_dim().first, get_color("arrow_fg"));
}
...
void Edge::draw_arrow_triangle(ModifiedPGE& engine) const {
    ...
    int x_base, y_base;
    if(vec_edge_x != 0) {
        float coefficient = -sqrt((base_distance * base_distance) / (float)(vec_edge_x * vec_edge_x + vec_edge_y * vec_edge_y));
        x_base = intersection.first - coefficient * vec_edge_x;
        y_base = intersection.second - coefficient * vec_edge_y;
        float vec_perp_y = sqrt((arrow_distance * arrow_distance) / (float)((vec_edge_y * vec_edge_y)/(float)(vec_edge_x * vec_edge_x) + 1));
        float vec_perp_x = -(vec_edge_y/(float)vec_edge_x) * vec_perp_y;
        engine.FillTriangle(intersection.first, intersection.second, x_base + vec_perp_x, y_base + vec_perp_y, x_base - vec_perp_x, y_base - vec_perp_y, olc::RED);
    }
    else {
        if(vec_edge_y != 0) {
            x_base = intersection.first;
            y_base = intersection.second + (vec_edge_y/abs(vec_edge_y)) * base_distance;
...
void Edge::draw_debug_arrow_triangle(ModifiedPGE& engine) const {
    ...
    if(vec_edge_x != 0) {
        ...
        x_base = intersection.first - coefficient * vec_edge_x;
        y_base = intersection.second - coefficient * vec_edge_y;
        vec_perp_y = sqrt((arrow_distance * arrow_distance) / (float)((vec_edge_y * vec_edge_y)/(float)(vec_edge_x * vec_edge_x) + 1));
