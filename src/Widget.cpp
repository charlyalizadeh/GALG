...
int Widget::get_y() const { return m_coord.second; }
std::pair<int, int> Widget::get_coord() const { return m_coord; }
int Widget::get_width() const { return m_dim.first; }
int Widget::get_height() const { return m_dim.second; }
std::pair<int, int> Widget::get_dim() const { return m_dim; }
olc::Pixel Widget::get_color(const std::string& label) const { return m_colors.at(label); }
bool Widget::get_is_drag() const { return m_is_drag; }
...
void Widget::set_x(const int& x) { m_coord.first = x; }
void Widget::set_y(const int& y) { m_coord.second = y; }
void Widget::set_coord(const std::pair<int, int>& coord) { m_coord = coord; }
void Widget::set_coord(const int& x, const int& y) { m_coord = {x, y}; }
void Widget::set_width(const int& width) { m_dim.first = width; }
void Widget::set_height(const int& height) { m_dim.second = height; }
void Widget::set_dim(const std::pair<int, int>& dim) { m_dim = dim; }
void Widget::set_dim(const int& width, const int& height) { m_dim = {width, height}; }
void Widget::set_is_drag(const bool& drag) { m_is_drag = drag; }
void Widget::set_coord_drag(const std::pair<int, int>& coord_drag) { m_coord_drag = coord_drag; }
void Widget::set_gui_state(const std::string& gui, const bool& state) { m_gui_state[gui] = state; }
void Widget::set_mouse_pos(const std::pair<float, float>& mouse_pos) { m_mouse_pos = mouse_pos; }
void Widget::set_id(boost::uuids::uuid id) { m_id = id; }
...
bool Widget::is_mouse_over(ModifiedPGE& engine) const  {
    int mouse_x = engine.GetMouseX(); 
    int mouse_y = engine.GetMouseY();
    return mouse_x >= m_coord.first && mouse_y >= m_coord.second &&
        mouse_x <= m_coord.first + m_dim.first && mouse_y <= m_coord.second + m_dim.second;
}
...
bool Widget::is_shift_held(ModifiedPGE& engine) const {
    return engine.GetKey(olc::Key::SHIFT).bHeld || engine.GetKey(olc::Key::SHIFT).bPressed;
}
bool Widget::is_ctrl_held(ModifiedPGE& engine) const {
    return engine.GetKey(olc::Key::CTRL).bHeld || engine.GetKey(olc::Key::CTRL).bPressed;
}
...
void Widget::draw(ModifiedPGE& engine) const { engine.DrawString(0, 0, "ah"); }
