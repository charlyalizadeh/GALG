...
class Widget{
    public:
        ...
        Widget(const std::pair<int,int>& coord,
               ...
               const boost::uuids::uuid& m_id = boost::uuids::uuid());
        Widget();
        virtual ~Widget();
        ...
        const std::pair<int,int>& get_coord() const;
        const int& get_width() const;
        const int& get_height() const;
        const std::pair<int,int>& get_dim() const;
        const olc::Pixel& get_color(const std::string& label) const;
        const bool& get_is_drag() const;
        const std::pair<int,int>& get_coord_drag() const;
        ...
        // Color
        void add_color(const std::string& label,
                       const olc::Pixel& color);
        void add_color(const std::string& label,
                       const std::string& hexadecimal_color);
        void add_color(const std::string& label,
                       const int& red,
                       const int& green,
                       const int& blue);
        ...
        virtual bool is_clicked(ModifiedPGE& engine,
                                const int& button = 0) const;
        virtual bool is_held(ModifiedPGE& engine,
                             const int& button = 0) const;
        virtual bool is_shift_held(ModifiedPGE& engine) const;
        virtual bool is_ctrl_held(ModifiedPGE& engine) const;
        virtual bool is_released(ModifiedPGE& engine,
                                 const int& button = 0) const;
